...
double Store::get_mean(const std::string& name, const Query& query) {
    ...
    auto filtered_events = events | std::views::filter([&](const auto& event) {
                               return !query.from || event.date >= *query.from;
                           }) |
                           std::views::filter([&](const auto& event) {
                               return !query.to || event.date <= *query.to;
                           });

    auto values = filtered_events |
                  std::views::transform([](const auto& event) {
                      return std::views::all(event.values);
                  }) |
                  std::views::join;
